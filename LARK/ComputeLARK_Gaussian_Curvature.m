function LARK = ComputeLARK_Gaussian_Curvature(varargin)
...
[zxx,zxy]=gradient(zx);
[zyx,zyy]=gradient(zy);
g = (zxx.*zyy-zxy.*zyx)./((1+zx.^2+zy.^2).^1.5);
...
zxx = EdgeMirror(zxx,[win,win]);
zxy = EdgeMirror(zxy,[win,win]);
zyx = EdgeMirror(zyx,[win,win]);
zyy = EdgeMirror(zyy,[win,win]);
g = EdgeMirror(g,[win,win]);
...
%         G = [gx(:), gy(:)];
        ...
        gxx = zxx(i:i+wsize-1, j:j+wsize-1).* K;
        gxy = zxy(i:i+wsize-1, j:j+wsize-1).* K;
        gyx = zyx(i:i+wsize-1, j:j+wsize-1).* K;
        gyy = zyy(i:i+wsize-1, j:j+wsize-1).* K;
        gg = g(i:i+wsize-1, j:j+wsize-1).* K;
        G2 = [gxx(:), gxy(:) gyx(:), gyy(:) gg(:)];
